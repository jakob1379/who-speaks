...
def test_sample_media_pipeline_smoke(sample_media_path: Path) -> None:
    ...
    assert result.display_name == sample_media_path.name
    assert result.media_identity == sample_media_path.name
    ...
    assert result.metadata.opencv_available is True, (
        f"OpenCV failed to import in the sample smoke environment: {result.metadata.opencv_import_error}"
    )
